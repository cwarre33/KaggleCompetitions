"""Orbit Wars agent — ProducerLite strategy.

Compatible with the kaggle_environments driver and the local
``orbit_lite`` evaluation harness.
"""
...
# Make the sibling ``orbit_lite`` package importable however this script is
# invoked: loaded in place, submitted at a tar-archive root, or exec'd by
...
class ProducerLiteConfig:
    """Behaviour knobs for the ProducerLite planner (2P defaults)."""

    # planning window — drives projection, movement build, and ETA cap
    horizon: int = 18
    ...
    roi_threshold: float       = 1.5  # fire only if score exceeds this
    ...
    # FFA bonus terms (zero-out harmlessly in 2P)
...
def _movement_config(config: ProducerLiteConfig, *, player_count: int) -> MovementConfig:
    """Return a MovementConfig with fleet tracking on and horizon = config.horizon."""
...
def cheap_enemy_pressure(obs, cache, *, horizon: float, player_id: int) -> Tensor:
    """Distance-decayed enemy-mass proxy per planet — shape ``[P]``.

    Used only as the **regroup gradient**: owned planets are ranked by how
    stressed they are and ships flow up the gradient.  For each planet ``t``,
    sums a distance-decayed share of every enemy source's *current* garrison
    that could straight-line reach ``t`` within ``horizon`` turns, using the
    step-0 centre distance ``cross_dist[0]``.  The decay
    ``(1 − d / (speed·H))₊`` weights nearer threats more, giving a graded
    frontline signal in ship-mass units.

    Approximations: orbital drift over the horizon, production accrued in
    flight, per-owner split, and in-flight enemy fleets are all ignored.
    Pure arithmetic on cached tensors — intentionally cheap.
    """
    P      = int(obs.P)
    device = obs.device
    ...
    ships      = obs.ships.to(dtype)                                    # [P]
    speeds     = fleet_speed(ships.clamp(min=1e-6))                     # [P]
    reach_dist = (speeds.view(P, 1) * float(horizon)).clamp(min=1e-6)  # [P, 1]

    enemy  = obs.alive & (obs.owner_abs >= 0) & (obs.owner_abs != int(player_id))
    eye    = torch.eye(P, device=device, dtype=torch.bool)
    valid  = enemy.view(P, 1) & obs.alive.view(1, P) & ~eye            # [P, P]
...
def plan_lite_waves(
    *,
    ...
    """Single-size, single-source attack planner + optional regroup pass.

    Builds exactly one candidate per ``(source, target)`` shortlist pair —
    fleet size = the source's max garrison launch (``safe_drain``) — scores
    them with the exact competitive flow diff, then greedily fires the best
    wave per target up to ``max_waves_per_turn``.  Returns the combined
    ``LaunchEntries`` (attack waves ++ regroup).
    """
    P      = obs.P
    device = obs.device
    dtype  = obs.ships.dtype
    ...
    # Single fleet size = max garrison launch; engine needs integer counts.
    sizes = drain.view(S, 1).expand(S, T).floor()                       # [S, T]

    # ── reachability & intercept geometry ─────────────────────────────────
    # Strict-superset precheck: skip body screen for physically unreachable pairs.
    ...
    # Gate on capture floor at arrival turn (defenders grow with k).
    ...
    # ── pack candidates (one per source–target pair, contributor axis L=1) ─
    L = 1
    C = S * T
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    """Full per-turn pipeline: build movement → plan waves + regroup → emit.

    ``memory`` must expose a mutable ``movement`` attribute (rolling cache).
    """
    ...
    launches = infer_planned_launches_from_entries(
        ...
        entries=entries, player_id=int(obs.player_id),
    )
    apply_private_planned_launches(
...
# 4-player FFA preset — only the knobs that differ from the 2P default.
...
def agent(obs):
    """Single-observation entry point for local play and Kaggle submission."""
