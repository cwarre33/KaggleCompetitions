...
def build_modes(world):
    ship_domination = (world.my_total - world.enemy_total) / max(1, world.my_total + world.enemy_total)
    prod_domination = (world.my_prod - world.enemy_prod) / max(1, world.my_prod + world.enemy_prod)
    # Ship counts are the current scoreboard; production is a leading indicator of
    # where ship counts are heading. Blending avoids flipping defensive when we're
    # ship-behind but production-ahead (about to overtake), and vice versa.
    domination = 0.7 * ship_domination + 0.3 * prod_domination
